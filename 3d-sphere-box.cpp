	...
	for (int t = 0; t < 10000; t++) {
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				...
				vec3 ro = vec3(-10, -1, 0); // camara position x y z
				vec3 rd = norm(vec3(2, uv)); // (zoom, dinamic position x y on pixel matrix)
				ro = rotateY(ro, 0.30); // angle of camara position by center of space
				rd = rotateY(rd, 0.30); 
				ro = rotateZ(ro, t * 0.01); // camara's speed of rotating
				rd = rotateZ(rd, t * 0.01); // speed of rotating of objects
				float diff = 1;
				for (int k = 0; k < 5; k++) {
					float minIt = 99999;
				...
				int color = (int)(diff * 10); // quantity of pixel colors
